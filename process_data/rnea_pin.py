...
#vinc data
if which == 'Vinc':
    path_grf = f"DATA/{which}/{subject}/{task}_forces.csv"
    path_joint = f"DATA/{which}/{subject}/{task}_joints.csv"
    ...
    center_fp1 = pf1_corners.mean(axis=0)/1000
    center_fp2 = pf2_corners.mean(axis=0)/1000
...
#hum data
elif which == 'HUMANOIDS':
    path_grf = f"DATA/{which}/{subject}/{task}_kinetics_global.csv"
    path_joint = f"DATA/{which}/{subject}/{task}_joints.csv"

    urdf_path = f"DATA//urdf_scaled/{which}/{subject}.urdf"
...
initial_M1 = grf_df.loc[0, [mx1, my1, mz1]].values / 1000.0
initial_M2 = grf_df.loc[0, [mx2, my2, mz2]].values / 1000.0
M_total_initial = initial_M1 + initial_M2
...
for i in range(n_samples):
    ...
    pf1_forces_clean[i]  = F1_clean
    pf1_moments_clean[i] = M1_clean

    pf2_forces_clean[i]  = F2_clean
    pf2_moments_clean[i] = M2_clean
    
    
    #first proposition for moment : moment_pf = moment_pf + (moment_rnea[0] - moment_pf[0])
    # offset_M = rnea_moments_world[0, :3] - M_total_initial
    # pf_moments_world_clean[i, 0] = pf_moments_world_clean[i, 0] + offset_M[0] # Mx
    # pf_moments_world_clean[i, 1] = pf_moments_world_clean[i, 1] + offset_M[1] # My


##second proposition moment_pf = moment_pf + offset_moment
mean_mrnea = np.mean(rnea_moments_world, axis=0)
mean_mpf = np.mean(pf_moments_world_clean, axis=0)

offset_Mx = mean_mrnea[0] - mean_mpf[0]
offset_My = mean_mrnea[1] - mean_mpf[1]
offset_Mz = -np.mean(pf_moments_world_clean[:, 2])

# for i in range(n_samples):
#     pf_moments_world_clean[i, 0] = pf_moments_world_clean[i, 0] + offset_Mx # Mx
#     pf_moments_world_clean[i, 1] = pf_moments_world_clean[i, 1] + offset_My # My

# ##moment_z ~ 0
pf_moments_world_clean[:, 2] = pf_moments_world_clean[:, 2]+offset_Mz
...
for i in range(n_samples):
    ...
    pf_moments_world_clean[i] = pf1_moments_clean[i] + pf2_moments_clean[i]
...
for i in range(n_samples):
    ...
    pos_bassin = q_ref[i, 0:3]                         # vecteur OP (Position bassin)
...
plt.suptitle(f"{subject} {task} global", fontsize=16)
plt.tight_layout(rect=[0, 0.03, 1, 0.95])


##########################################################################
err_f = np.mean(np.abs(rnea_forces_pelvis - pf_forces_pelvis[:n_samples]), axis=0)
err_m = np.mean(np.abs(rnea_moments_pelvis - pf_moments_pelvis), axis=0)

rmse_f = np.sqrt(np.mean((rnea_forces_pelvis - pf_forces_pelvis[:n_samples])**2, axis=0))
rmse_m = np.sqrt(np.mean((rnea_moments_pelvis - pf_moments_pelvis)**2, axis=0))

print(f"MAE  FORCES  PELVIS (N)   -> X: {err_f[0]:.2f}, Y: {err_f[1]:.2f}, Z: {err_f[2]:.2f}")
print(f"MAE  MOMENTS PELVIS(Nm)  -> X: {err_m[0]:.2f}, Y: {err_m[1]:.2f}, Z: {err_m[2]:.2f}")
print(f"RMSE FORCES PELVIS (N)   -> X: {rmse_f[0]:.2f}, Y: {rmse_f[1]:.2f}, Z: {rmse_f[2]:.2f}")
print(f"RMSE MOMENTS PELVIS(Nm)  -> X: {rmse_m[0]:.2f}, Y: {rmse_m[1]:.2f}, Z: {rmse_m[2]:.2f}")
