#get medial markers for Vinc dataset (viz and save the data)
import sys
import os
import time
import numpy as np
...
from utils.model_utils import (
    ...
    knee_joint_center, ankle_joint_center, medial_from_ajc, as_col, create_virtual_foot_marker,create_virtual_MTOE, elbow_joint_center
)

# === Parameters ===
TRIAL = "Trial111"
subjects = ['Maria', 'Serge', 'Subject1', 
                'Vincent'] #'Christine', 'Jeremy', 'Jovana', 
...
FOOT_WIDTH = 0.05  #toe to 5th toe
ELBOW_LEN = 0.035#(ELBOW_WIDTH/2)

for subject in subjects:
    subject_dir = f"DATA/Vinc/{subject}"

    for task in os.listdir(subject_dir):

        task_dir = os.path.join(subject_dir, task)
        force_dir =os.path.join(subject_dir, task)
        if not task.endswith("filled.csv"):
            continue
        else:
            print(task_dir)
            mks_csv = task_dir
        
        cop_csv = f"DATA/Vinc/Christine/{TRIAL}_forces.csv"
        ...
        mks_dict, start_sample_dict = read_mks_data(df_mks, start_sample=0, converter=CONVERTER) ## changer X Y Z
        ...
        # Joints
        joint_names = [
            "rkne_jcp", "lkne_jcp", "rank_jcp", "lank_jcp", "lelb_jcp","relb_jcp",
            "RHJC", "LHJC", "pelvis_jcp", "rmkne", "lmkne", "rmank", "lmank","rMTOE","lMTOE", "lMELB","rMELB" ]
        ...
        def compute_joint_centers(frame, side):
            ...
            if side == "right":
                ...
                hip_label, knee_label, thigh_label, ankle_label, tib_label,toe_label, heel_label,sho_label,elb_label,lwr_label,mwr_label = (
                    "RHJC", "RKNE", "RTHI", "RANK", "RTIB", "RTOE", "RHEE","RSHO","RELB","RWRA","RWRB"
                )
            else:
                get_thigh_pose = get_thighL_pose
                hip_label, knee_label, thigh_label, ankle_label, tib_label,toe_label, heel_label,sho_label,elb_label,lwr_label,mwr_label = (
                    "LHJC", "LKNE", "LTHI", "LANK", "LTIB", "LTOE", "LHEE","LSHO","LELB","LWRA","LWRB"
                )
            SJC = frame[sho_label]
            ELB_lat = frame[elb_label]
            LWRA = frame[lwr_label]
            LWRB = frame[mwr_label]
            EJC = elbow_joint_center(SJC, ELB_lat, LWRA, LWRB,ELBOW_LEN, side=side)
            MELB = medial_from_ajc(EJC, ELB_lat, ELBOW_WIDTH)
            # Hip
            ...
            return {
                ...
                "MKNE": MKNE, "AJC": AJC, "MANK": MANK, "MTOE": MTOE, "EJC": EJC, "MELB":MELB
            }
        ...
        LMELB_buf = np.full((n_frames, 3), np.nan)
        RMELB_buf = np.full((n_frames, 3), np.nan)
        # === Animation ===
        for i in range(n_frames):
            ...
            for side in ("right","left"):
                ...
                place(vis, f"{prefix}elb_jcp", joints["EJC"])
                place(vis, f"{prefix}MELB", joints["MELB"])
        ...
        write_vec(df_mks, "LMELB", LMELB_buf)
        write_vec(df_mks, "RMELB", RMELB_buf)
