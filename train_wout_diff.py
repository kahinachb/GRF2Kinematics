...
def train_one_epoch(model, loader, optimizer, scheduler,device):
    ...
    for joints, kinetics in loader:
        joints   = joints.to(device)    # (B, T, 29)  ← target
        kinetics = kinetics.to(device)  # (B, T, 12)  ← input

        joints_pred = model(kinetics)   # (B, T, 29)
        loss = F.mse_loss(joints_pred, joints)
...
def evaluate(model, loader, device):
    ...
    for joints, kinetics in loader:
        ...
        joints_pred = model(kinetics)                        
        total_loss += F.mse_loss(joints_pred, joints).item() 
