"""Inference for one Anais-task model trained with morphology conditioning.

Paired with ``train_fm_full_step2motion_anais_feet_morphology.py``.  The
script extracts the same 12 segment-length features from ``DATA/10_urdf`` and
otherwise follows the existing local-foot inference procedure: multi-seed
Euler/Heun sampling, overlap inpainting, optional Butterworth filtering, CSV
metrics, and figures.
"""
...
import fm_inference_real_feet_morphology as common_morphology
...
def resolve_paths(results_dir: Path, model_path, scalers_path, model_label: str):
    if model_label not in {"ema", "raw"}:
        raise ValueError("--model-label must be either 'ema' or 'raw'.")
    model_path = (
        Path(model_path)
        if model_path is not None
        else results_dir / f"fm_anais_task_morphology_model_best_{model_label}.pth"
    )
    scalers_path = (
        Path(scalers_path)
        if scalers_path is not None
        else results_dir / "scalers_anais_task_morphology.json"
    )
...
def run_inference(
    ...
    """Run multi-seed full-trial inference for an Anais morphology model."""
    task = task_base.canonical_task_name(task)
    if task is None:
        raise ValueError(f"Unsupported task. Choose one of: {', '.join(task_base.SUPPORTED_TASKS)}")
    if task_base.canonical_task_name(trial_name) != task:
        raise ValueError(
    ...
    results_dir = Path(output_dir) if output_dir is not None else Path(f"results_anais_{task}_feet_morphology")
    model_path, scalers_path = resolve_paths(results_dir, model_path, scalers_path, model_label)
    ...
    print("\n" + "=" * 74)
    print("FLOW MATCHING INFERENCE — ANAIS / LOCAL FOOT FRAMES + MORPHOLOGY")
    ...
    stats = common_morphology.load_stats(scalers_path)
    model = common_morphology.load_morphology_model(model_path, device)
    ...
    for seed in range(n_seeds):
        ...
        reference, prediction = common_morphology.predict_full_trial_with_solver(
    ...
    prefix = f"{subject_name}_{trial_name}_anais_{task}_feet_morphology_{model_label}_{solver}"
    raw_seed0 = raw_predictions[0]
    np.savetxt(
        results_dir / f"{prefix}_prediction_raw.csv", raw_seed0, delimiter=",",
        header=",".join(common.JOINT_NAMES), comments="",
    )
    ...
    common.plot_prediction_groups(
        ...
        f"{trial_name}_anais_{task}_feet_morphology_{model_label}_{solver}", "raw",
    )
    ...
    if filtered_metrics is not None:
        ...
        np.savetxt(
            results_dir / f"{prefix}_prediction_filtered.csv", filtered_seed0, delimiter=",",
            header=",".join(common.JOINT_NAMES), comments="",
        )
        ...
        common.plot_prediction_groups(
            ...
            f"{trial_name}_anais_{task}_feet_morphology_{model_label}_{solver}", "filtered",
        )
        print(
