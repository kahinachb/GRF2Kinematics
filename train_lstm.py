...
# 1. DATASET 
# ==========================================
class BiomechDirectDataset(Dataset):
    def __init__(self, file_list, window_size=40, stats=None):
        ...
        for f_path, j_path in file_list:
            ...
            # Correction : Many-to-one (40 frames in -> 1 frame out) avec stride de 1
            for i in range(len(f_data) - window_size):
                self.samples.append((f_data[i : i + window_size], j_data[i + window_size - 1]))
        self.stats = stats
...
class BiLSTM_MLP(nn.Module):
    def __init__(self, input_dim=18, hidden_lstm=32, output_dim=12):
...
# ==========================================
def predict_full_trial(model, f_path, j_path, stats, device, window_size=40):
    model.eval()
    ...
    f_padded = np.vstack([np.tile(f_norm[0], (window_size - 1, 1)), f_norm])
    ...
    final_pred = (pred_j_norm * stats['j_s']) + stats['j_m']
...
def run_experiment():
    ...
    # Correction : Utilisation du nouveau nom de la classe Dataset
    train_loader = DataLoader(BiomechDirectDataset(train_pairs, stats=stats), batch_size=64, shuffle=True)
    val_loader = DataLoader(BiomechDirectDataset(get_pairs(val_subs), stats=stats), batch_size=64)

    model = BiLSTM_MLP(input_dim=18, output_dim=12).to(device)
    ...
    ref_full, pred_full = predict_full_trial(model, random_trial[0], random_trial[1], stats, device)
