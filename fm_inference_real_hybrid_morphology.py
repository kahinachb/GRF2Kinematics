"""Inference for the hybrid global + local-foot + morphology Flow model."""
...
import train_fm_full_step2motion_real_hybrid_morphology as hybrid
from fm_inference_real_feet_morphology import (
    JOINT_NAMES,
    aggregate_metrics,
    lowpass_prediction,
...
def load_stats(scalers_path: Path):
    """Load scalers and reject files from the local-only model."""
    with open(scalers_path, "r") as file:
        payload = json.load(file)
    required = {
        "condition_mean", "condition_std", "j_mean", "j_std",
        "morph_mean", "morph_std", "condition_layout", "morphology_feature_names",
    }
    missing = required.difference(payload)
    if missing:
        raise ValueError(f"Hybrid scalers at {scalers_path} are missing: {sorted(missing)}")
    if payload["condition_layout"] != "[global: RF,R M,R CoP,L F,L M,L CoP][feet: same order]":
        raise ValueError("Scaler condition layout does not match the hybrid training script.")
    if payload["morphology_feature_names"] != list(hybrid.MORPHOLOGY_FEATURE_NAMES):
        raise ValueError("Morphology feature order differs from the hybrid training script.")

    stats = {
        key: torch.tensor(payload[key], dtype=torch.float32)
        for key in ("condition_mean", "condition_std", "j_mean", "j_std", "morph_mean", "morph_std")
    }
    expected_shapes = {
        "condition_mean": (hybrid.N_CONDITION_FEATURES,),
        "condition_std": (hybrid.N_CONDITION_FEATURES,),
        "j_mean": (hybrid.N_JOINTS,),
        "j_std": (hybrid.N_JOINTS,),
        "morph_mean": (hybrid.N_MORPHOLOGY_FEATURES,),
        "morph_std": (hybrid.N_MORPHOLOGY_FEATURES,),
    }
...
def load_hybrid_model(model_path: Path, device):
    checkpoint = torch.load(model_path, map_location=device)
    state_dict = checkpoint.get("model", checkpoint) if isinstance(checkpoint, dict) else checkpoint
    model = hybrid.HybridMorphologyFlowTransformer().to(device)
...
def predict_full_trial_with_solver(
    ...
    stride=hybrid.STRIDE,
    n_steps=hybrid.INFERENCE_STEPS,
):
    """Hybrid full-trial Euler/Heun integration with overlap inpainting."""
    solver = solver.lower()
    ...
    model.eval()
    body_weight_cache = {}
    condition, reference = hybrid.load_hybrid_trial(
        global_path, feet_path, joints_path, body_weight_cache
    )
    condition = (torch.from_numpy(condition) - stats["condition_mean"]) / (stats["condition_std"] + 1e-6)
    morphology_values = morphology.subject_morphology(subject_name, {})
    ...
    full_prediction = torch.zeros((len(condition_for_sampling), hybrid.N_JOINTS), device=device)
    previous_start = None
    nfe_per_window = n_steps * (1 if solver == "euler" else 2)
    print(
        f"  [INF] Sampling hybrid Flow Matching ({solver.capitalize()}): "
        f"{total_frames} frames, {n_steps} steps/window, {nfe_per_window} NFE/window"
    )
    for start in hybrid.base.window_starts(len(condition_for_sampling), window_size, stride):
        ...
        x0 = torch.randn((1, window_size, hybrid.N_JOINTS), device=device)
        x = x0.clone()
...
def run_inference(
    subject_name,
    trial_name,
    model_path="results_real_hybrid_morphology/fm_hybrid_morphology_model_best_ema.pth",
    scalers_path="results_real_hybrid_morphology/scalers_hybrid_morphology.json",
    data_root=None,
    output_dir="results_real_hybrid_morphology",
    model_label="ema",
    solver="heun",
    n_steps=hybrid.INFERENCE_STEPS,
    n_seeds=3,
    ...
    data_root = Path(hybrid.DATA_ROOT if data_root is None else data_root)
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    data_path = data_root / subject_name / trial_name
    global_path = data_path / "kinetics_glob.npy"
    feet_path = data_path / "kinetics_feet.npy"
    joints_path = data_path / "all_joints.npy"
    if not global_path.exists() or not feet_path.exists() or not joints_path.exists():
        raise FileNotFoundError(
            f"Expected kinetics_glob.npy, kinetics_feet.npy, and all_joints.npy in {data_path}"
        )

    nfe_per_window = n_steps * (1 if solver == "euler" else 2)
    print("\n" + "=" * 76)
    print("FLOW MATCHING INFERENCE — HYBRID GLOBAL + FEET + MORPHOLOGY")
    ...
    model = load_hybrid_model(Path(model_path), device)
    print(f"[1/4] Model loaded: {model_path}")
    print(f"[2/4] Data: {data_path}")
    ...
    for seed in range(n_seeds):
        ...
        reference, prediction = predict_full_trial_with_solver(
            model, global_path, feet_path, joints_path, subject_name, stats, device,
    ...
    prefix = f"{subject_name}_{trial_name}_hybrid_morphology_{model_label}_{solver}"
    ...
    plot_prediction_groups(
        ...
        f"{trial_name}_hybrid_morphology_{model_label}_{solver}", "raw",
    )
    ...
    if apply_lowpass:
        ...
        plot_prediction_groups(
            ...
            f"{trial_name}_hybrid_morphology_{model_label}_{solver}", "filtered",
        )
        print(
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Inference for the hybrid global + feet + morphology model.")
    parser.add_argument("--subject", default="Christine")
    parser.add_argument("--trial", default="Trial110")
    parser.add_argument("--model", default="results_real_hybrid_morphology/fm_hybrid_morphology_model_best_ema.pth")
    parser.add_argument("--scalers", default="results_real_hybrid_morphology/scalers_hybrid_morphology.json")
    parser.add_argument("--data-root", default=None, help="Defaults to DATA_ROOT from the hybrid training script.")
    parser.add_argument("--output-dir", default="results_real_hybrid_morphology")
    ...
    parser.add_argument("--steps", type=int, default=hybrid.INFERENCE_STEPS)
